...
describe Toyclone do
  ...
  it "parses fit command" do
    config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv"])
  ...
  describe "fit-vi command options" do
    it "parses -c / --num-clusters" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "-c", "40"])
      config.num_clusters.should eq(40)
    end

    it "parses -d binomial" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "-d", "binomial"])
      config.density.should eq(Toyclone::Density::Binomial)
    end

    it "parses -d beta-binomial" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "-d", "beta-binomial"])
      config.density.should eq(Toyclone::Density::BetaBinomial)
    end

    it "raises on invalid density name" do
      expect_raises(Toyclone::CliError, /Invalid density/) do
        Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "-d", "gaussian"])
      end
    end

    it "parses -g / --num-grid-points" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "-g", "50"])
      config.num_grid_points.should eq(50)
    end

    it "parses -r / --num-restarts" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "-r", "5"])
      config.num_restarts.should eq(5)
    end

    it "parses --max-iters" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "--max-iters=500"])
      config.max_iters.should eq(500)
    end

    it "parses --seed" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "--seed=42"])
      config.seed.should eq(42_u64)
    end

    it "parses --precision" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "--precision=500.0"])
      config.precision.should eq(500.0)
    end

    it "parses --convergence-threshold" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "--convergence-threshold=1e-4"])
      config.convergence_threshold.should be_close(1e-4, 1e-12)
    end

    it "parses --mix-weight-prior" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "--mix-weight-prior=2.0"])
      config.mix_weight_prior.should eq(2.0)
    end

    it "parses --kernel-threads" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "--kernel-threads=4"])
      config.kernel_threads.should eq(4)
    end

    it "parses --restart-parallelism" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "--restart-parallelism=2"])
      config.restart_parallelism.should eq(2)
    end

    it "parses --compress" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "--compress"])
      config.compress?.should be_true
    end

    it "parses --print-freq" do
      config = Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "--print-freq=50"])
      config.print_freq.should eq(50)
    end

    it "raises when --in-file is omitted" do
      expect_raises(Toyclone::CliError) do
        Toyclone::CLI.parse(["fit-vi", "-o", "out.tsv"])
      end
    end

    it "raises when --out-file is omitted" do
      expect_raises(Toyclone::CliError) do
        Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv"])
      end
    end

    it "raises on unknown option" do
      expect_raises(Toyclone::CliError, /is not a valid option/) do
        Toyclone::CLI.parse(["fit-vi", "-i", "in.tsv", "-o", "out.tsv", "--unknown"])
      end
    end

    it "fit --help shows fit-specific usage" do
      config = Toyclone::CLI.parse(["fit-vi", "--help"])
      config.action.should eq(Toyclone::Action::Help)
  ...
  describe "fit-mcmc command options" do
    it "parses fit-mcmc command and sets engine" do
      config = Toyclone::CLI.parse(["fit-mcmc", "-i", "in.tsv", "-o", "out.tsv"])
      config.command.should eq("fit-mcmc")
      config.engine.should eq(Toyclone::Engine::MCMC)
    end

    it "parses MCMC-specific options" do
      config = Toyclone::CLI.parse([
        "fit-mcmc",
        "-i", "in.tsv",
        "-o", "out.tsv",
        "--num-iters=200",
    ...
    it "fit-mcmc --help shows MCMC-specific usage" do
      config = Toyclone::CLI.parse(["fit-mcmc", "--help"])
      config.action.should eq(Toyclone::Action::Help)
    ...
    it "rejects an invalid init-method" do
      expect_raises(Toyclone::CliError, /Invalid init-method/) do
        Toyclone::CLI.parse(["fit-mcmc", "-i", "in.tsv", "-o", "out.tsv", "--init-method=other"])
      end
    end
  end
end
